...
- 배포본은 romfs 파일 23개를 덮어씌우는 LayeredFS 패치와, 일본판 CIA·3DS 파일을 한글판으로 바꾸는 패처 두 가지입니다.
...
- 패처로 암호화된 원본을 바꿀 때는 `boot9.bin`과 `seeddb.bin`(GodMode9으로 덤프). 이미 복호화된 파일은 필요 없습니다.

### 적용 방법

[배포 페이지](../../releases/latest)에서 둘 중 하나를 받습니다.

**LayeredFS (`EverOasis_KO_v0.2_LayeredFS.zip`)**
...
**패처 (`EverOasis_KO_v0.2_Patcher.zip`)**

1. ZIP을 폴더째 풀고, 일본판 `.cia` 또는 `.3ds` 파일을 `패치하기.bat`에 끌어다 놓습니다.
2. 원본과 같은 폴더에 `원래 이름_KO.cia` 또는 `원래 이름_KO.3ds`가 생깁니다. 원본 파일은 바뀌지 않습니다.
3. 3DS 실기에는 만든 CIA를 설치합니다. LayeredFS 폴더는 필요 없습니다. 3DS 파일은 복호화된 상태라 에뮬레이터·플래시카트용입니다.

패처는 원본을 풀어 게임 안 한글 파일 23개와 HOME 메뉴 배너·게임 이름을 바꾼 뒤 다시 묶습니다. 넣은 파일이 일본판 본편인지, 바꿀 파일이 원본 그대로인지 먼저 확인합니다. 파이썬이 함께 들어 있어 따로 설치할 것은 없습니다.
