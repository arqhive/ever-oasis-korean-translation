# -*- coding: utf-8 -*-
"""에버 오아시스 한글 패치 적용기 — 일본판 CIA / 3DS 파일을 한글판으로 만든다.
...
def ask():
    print('\n일본판 에버 오아시스 .cia 또는 .3ds 파일 경로를 입력하세요 (파일을 이 창에 끌어다 놓아도 됩니다).')
    return input('> ').strip().strip('"')


def main():
    man = json.load(open(os.path.join(PAYLOAD, 'manifest.json'), encoding='utf-8'))
    print('에버 오아시스 한글 패치 %s (CIA·3DS 패처)\n' % man['version'])
    ...
    for src in files:
        ...
        if not os.path.isfile(src):
            print('[오류] 파일을 찾을 수 없습니다: %s' % src); continue
        stem, ext = os.path.splitext(src)
        if ext.lower() not in ('.cia', '.3ds', '.cci'):
            print('[오류] .cia 또는 .3ds 파일만 넣을 수 있습니다.'); continue
        dst = stem + '_KO' + ('.cia' if ext.lower() == '.cia' else '.3ds')
        t = time.time()
        try:
            eopatch.patch(src, dst, PAYLOAD, os.path.join(HERE, 'bin'), os.path.join(TOP, 'work'),
                          key_extra=(TOP, HERE))
        except SystemExit as e:
            print('[오류] %s' % e); continue
        except Exception:
            traceback.print_exc(); print('[오류] 예상하지 못한 문제가 생겼습니다.'); continue
        print('\n완료 (%d초): %s' % (time.time() - t, dst)); ok += 1
